on run argv
    set targetUrl to "data:text/html,<title>OmniControl Safari Open</title><p>OmniControl Safari Open</p>"
    ...
    try
        ...
        tell application "Safari"
            set payload to do JavaScript "JSON.stringify({title: document.title, href: location.href, readyState: document.readyState})" in front document
        end tell
        return payload
    on error errMsg
